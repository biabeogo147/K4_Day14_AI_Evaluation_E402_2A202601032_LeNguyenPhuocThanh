...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"{criteria_lines}\n"
            "Respond with a JSON object mapping each criterion name to a score."
        )
        ...
        try:
            ...
            scores = {name: float(parsed[name]) for name in rubric}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if item_scores:
                averages.append(sum(item_scores.values()) / len(item_scores))
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def _avgs(results: list) -> tuple[float, float, float]:
            n = len(results)
            if n == 0:
                return 0.0, 0.0, 0.0
            return (
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest = [metric for metric, score in scores.items() if score == min_score]
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_map = {
            ...
            "irrelevant": "Improve prompt clarity and intent routing so answers directly address the question",
            "incomplete": "Increase chunk size or top-k in the RAG pipeline to reduce context fragmentation",
            "off_topic": "Add few-shot examples showing complete, on-topic answers to guide generation",
            "refusal": "Loosen overly strict guardrails so the agent answers in-scope questions confidently",
        }
        ...
        generic_fallbacks = [
            "Add more diverse examples to the golden dataset to catch edge cases earlier",
            "Review retrieval ranking so the most relevant chunks surface first",
            "Run regular regression tests to catch metric drops before deployment",
        ]
